...
description: "Android ADB 快速工具是一套免費、免安裝的 Windows 圖形化 ADB 工具，支援 USB／Wi-Fi 偵錯、APK 組合與拖放安裝、亮度與快速設定、手機截圖，以及 DCIM 壓縮或增量資料夾備份。"
...
- 支援 USB 連線及程式內直接進行 Wi-Fi 配對、連線與中斷，並提供 mDNS 搜尋、自動重新連線及相容性檢查。
- 同時連接多台手機時，可依型號、序號與 USB／Wi-Fi 標記選擇操作裝置，或將 APK 安裝到全部裝置。
- 建立多組「常用 APK 安裝」清單，一次依序安裝整組 App。
- 掃描程式旁 `APKs` 資料夾，自動建立與資料夾同步的 APK 組合。
- 把一個或多個 APK 拖進視窗後立即安裝。
- 讀取並即時調整手機亮度，支援滑桿、數字及鍵盤。
- 實測自動調整亮度功能可搭配 ArgyllCMS 與外接色度計，自動逼近指定亮度。
- 快速設定自動亮度、螢幕關閉時間、充電時保持亮屏及媒體音量。
- 在手機開啟指定網址，以及把目前手機畫面擷取為 PNG。
- 下載 `DCIM`、`Pictures`、`Picture` 內的資料，可打包成 ZIP 或直接保留目錄結構並進行分手機增量備份。
- 下載前先取得手機端檔案大小，略過超過指定上限的單一檔案。
- Android 的 `.thumbnails` 縮圖快取資料夾會直接忽略，不列入下載或錯誤統計。
...
主畫面的「Wi-Fi 連線」可直接輸入手機 IP、配對 Port、六位數配對碼及偵錯連線 Port，不必另外開啟命令提示字元。程式也會保留已配對裝置紀錄，可選擇啟動時自動重新連線，並能透過 mDNS 搜尋區域網路上的配對或偵錯服務；USB 操作步驟則保留在同一視窗的教學頁。

> **驗證狀態：** Wi-Fi 配對、mDNS 搜尋、自動重新連線及多裝置環境尚未完成足夠機型與網路環境的實機驗證，目前應視為測試中功能。實際結果可能受 Android 版本、廠牌客製系統、ADB 版本、路由器用戶端隔離及防火牆影響，建議保留 USB 偵錯作為備援。

同時連接多台手機時，連線區會出現裝置下拉選單，每個項目都會顯示型號、序號及 USB／Wi-Fi 連線方式。程式會記住上次操作的手機；亮度、快速設定、截圖、資料下載與快速傳輸只作用於下拉選單中的裝置，避免誤操作其他手機。

## 常用 APK 安裝：把一整套 App 變成一個按鈕

需要測試新手機或替多台設備安裝固定工具時，可以建立多個 APK 組合。例如建立「效能測試」、「Google 工具」及「展示用 App」，每組加入不同的 APK，之後選取組合並按「全部安裝」即可。

若勾選「APK 安裝到全部裝置」，常用組合與快速安裝都會逐台執行。完成視窗會依裝置分組顯示成功與失敗數量，清單則以「2/3 台成功」這類文字呈現彙整結果；其中一台失敗不會中止其他手機。

自訂組合可以：

- 新增、重新命名與刪除。
- 直接拖曳清單項目調整顯示順序，程式會記住排序。
- 雙擊自訂組合即可編輯名稱；資料夾同步組合維持由資料夾名稱管理。
- 按「加入 APK」選擇檔案。
- 直接把 APK 拖到右側清單加入。
- 移除選取項目，或允許降版安裝。
- 逐一顯示等待、安裝中、成功或失敗狀態。
...
若只是臨時安裝一、兩個 APK，不必先建立組合。切換到「快速安裝 / 傳輸」後，左側拖曳區維持 APK 快速安裝：將一個或多個 `.apk` 檔拖入並放開，程式就會立即開始安裝；也可以直接點一下左側區域選擇 APK。

右側則是快速傳輸區。可先選擇 `/sdcard/Download/`（預設）、`/sdcard/DCIM/`、`/sdcard/Pictures/` 或內部儲存根目錄 `/sdcard/`，再將任何檔案或整個資料夾拖入；程式會自動傳輸到所選位置。拖入資料夾時會保留最外層資料夾名稱與所有子目錄結構；多個項目會逐一處理，其中一項失敗也不會中止其餘傳輸。

![快速安裝頁面，可拖放一個或多個 APK 後立即安裝](images/01-quick-install.png)

這種設計除了適合把剛編譯完成的 App 丟進實機驗證，也方便臨時傳送測試資料、影片或整包素材。安裝與傳輸結果都會寫入「執行紀錄」，若 Android 回傳簽章不符、版本降級、空間不足或傳輸錯誤，也能從紀錄判斷原因。
...
- 點擊畫面上的 `－`、`＋`。
- 在亮度頁按實體鍵盤的 `-`、`+`。
按鍵每次調整 **1**，變更後會立即透過 ADB 套用。若勾選「調整時自動關閉自動亮度」，程式會先切換為手動亮度，避免系統感光機制馬上把數值改回去。

### 使用色度計全自動調整到指定 nit

程式在原有手動控制下方提供「實測自動調整亮度功能」。使用者可指定 ArgyllCMS 的 `spotread.exe`、選擇性的 `.ccss`／`.ccmx` 修正檔，輸入例如 **200 nit** 與容許誤差。程式會先讓手機顯示全白測試圖，再反覆進行「設定 Android 亮度 → 等待穩定 → 由色度計量測」；達標時保留結果，無法達標時則套用實測最接近的亮度並顯示誤差。
...
- 將媒體音量快速調到最低或最高。
- 輸入網址並在手機上開啟。
- 擷取目前手機畫面並儲存為 PNG，或按「截圖到剪貼簿」直接複製到 Windows 系統剪貼簿。
...
程式會先在手機端建立修改時間、檔案路徑與大小清單，再依使用者選擇執行以下方式：

- **ZIP 壓縮包**：每次完整下載，保留原本資料夾結構後壓縮成：

```text
手機型號_yyyyMMdd-HHmmss.zip
```

- **資料夾（保留結構）**：直接在選定位置建立 `DCIM`、`Pictures`、`Picture` 等原始目錄，不額外建立手機名稱資料夾，也不壓縮檔案。此模式預設啟用完整下載時間紀錄；下次在相同電腦目的地執行時，只下載紀錄時間之後新增或修改的檔案。

不同手機的下載紀錄分開保存，同型號也會透過穩定裝置 ID 區分。更換電腦儲存位置時會自動視為首次完整下載，也可按「重置目前手機紀錄」強制清除時間點。依大小上限正常略過的檔案不列入錯誤率；實際複製的檔案錯誤率在 10% 以下（含 10%）仍視為成功並更新時間，超過 10% 時則不更新，並顯示警告訊息。
...
Android 11 以上可在開發人員選項使用「無線偵錯」與六位數配對碼，並建議搭配最新版 Platform Tools；配對功能至少需要支援 `adb pair` 的 Platform Tools 30.0.0。Android 10 以下通常要先以 USB 完成偵錯授權，再執行 `adb tcpip 5555` 與無線連線。手機與電腦應位於可互通的同一網路。
...
其中包含 ADB 路徑、自訂 APK 組合、排序與最後選擇的組合、視窗大小、資料下載方式與位置、各手機的完整下載時間，以及檔案大小過濾設定。要將程式搬到另一台電腦時，APK 資料夾同步組合可直接隨程式複製；自訂清單若引用原電腦的絕對路徑，則需要重新指定檔案。Release 另提供 `-complete` 完整包，已內含 ADB 與 `spotread.exe`；沒有 `-complete` 後綴的標準包只包含 AndroidADBTools。
