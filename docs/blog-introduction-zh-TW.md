...
其中包含 ADB 路徑、自訂 APK 組合、排序與最後選擇的組合、視窗大小、資料下載方式與位置、各手機的完整下載時間，以及檔案大小過濾設定。要將程式搬到另一台電腦時，APK 資料夾同步組合可直接隨程式複製；自訂清單若引用原電腦的絕對路徑，則需要重新指定檔案。Release 同時提供標準可攜式版、內含 ADB 與 `spotread.exe` 的 `-complete` 可攜式版，以及具備開始功能表、桌面捷徑選項與解除安裝功能的 `-complete-setup.exe` 完整安裝版。
